...
///  微博数据列表模型
class StatusesData: NSObject, DictModel4SwiftProtocol {
    ///  微博记录数组
    var statuses: [Status]?
    ///  微博总数
    ...
    class func loadStatus(maxId: Int = 0,completion: (data: StatusesData?, error: NSError?)->()) {
        ...
        {
            let params = ["access_token": token, "max_id": "\(maxId)", "count": "\(8)"]

            // 发送网络异步请求
            net.requestJSON(HTTPMethod.GET, WB_Home_Timeline_URL, params) { (result, error) -> () in
                if error != nil {
                    // 错误处理
                ...
                // 字典转模型
                var data = DictModel4Swift.sharedInstance.objectWithDictionary(result as! NSDictionary, cls: StatusesData.self) as? StatusesData
                // 回调 -> 将模型通知给视图控制器
                // 如果有下载图像的 url，就先下载图像
...
///  微博模型
class Status: NSObject, DictModel4SwiftProtocol {
    ///  微博创建时间
...
class StatusPictureURL: NSObject {
    ...
    var large_pic: String?{
        get{
            var str = thumbnail_pic! as String
            return str.stringByReplacingOccurrencesOfString("thumbnail", withString: "large")
        }
    }
}
